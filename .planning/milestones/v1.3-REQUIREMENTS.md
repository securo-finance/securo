# Requirements Archive: v1.3 Gestão de Cartões
...
- [x] **CARD-01**: User can access a `Cartões` menu entry positioned below `Transações` in the main navigation.
- [x] **CARD-02**: User can view a list of cards imported from Pluggy in the `Cartões` screen.
- [x] **CARD-03**: User can identify each imported card in the `Cartões` list using the existing card metadata available in Flux.

### Card Transaction Browsing

- [x] **CARD-04**: User can open a dedicated transaction view for a selected card from the `Cartões` list.
- [x] **CARD-05**: User can view only transactions associated with the selected card in that card transaction view.
- [x] **CARD-06**: User can use a card transaction view that preserves the main `Transações` screen behavior where it remains compatible with card-scoped browsing.

### Snapshots and Filters

- [x] **CARD-07**: User can switch between available monthly snapshots while staying in the selected card transaction context.
- [x] **CARD-08**: User can filter the selected card's transactions by category.
- [x] **CARD-09**: User can combine snapshot selection and category filtering without leaving the selected card transaction context.
