...
@pytest.mark.asyncio
async def test_enable_banking_error_does_not_echo_saved_credential(
    client, admin_auth_headers, auth_headers
):
    secret = "saved-app-id-must-stay-private"
    response = await client.patch(
        ...
        json={
            "values": {
                ...
                "enable_banking_private_key": "stored-key-for-test",
            }
        },
    )
    assert response.status_code == 200
    ...
    def provider_client():
        return httpx.AsyncClient(
            ...
            transport=httpx.MockTransport(
                lambda request: httpx.Response(400, text=f"upstream echoed {secret}")
            ),
        )

    with patch("app.providers.enable_banking.EnableBankingProvider._client", side_effect=provider_client):
        response = await client.get(
            "/api/connections/enable_banking/institutions", headers=auth_headers
        )
...
async def test_private_key_fallback_requires_a_readable_nonempty_file(
    ...
    assert status["fields"]["enable_banking_private_key"]["environment_configured"] is bool(
        file_contents
    )
    ...
    assert next(p for p in available if p["name"] == "enable_banking")["configured"] is bool(
        file_contents
    )
...
async def test_private_key_file_takes_precedence_over_raw_environment_key(
    ...
    assert status["fields"]["enable_banking_private_key"]["environment_configured"] is False
    available = all_known_providers(await resolve_settings(session))
    assert (
        next(provider for provider in available if provider["name"] == "enable_banking")[
            "configured"
        ]
        is False
    )
...
@pytest.mark.asyncio
async def test_sync_rates_uses_saved_app_id(session, clean_db, monkeypatch):
    from app.providers.openexchangerates import OpenExchangeRatesProvider
    from app.services.fx_rate_service import sync_rates
    from app.services.provider_settings import update_provider

    monkeypatch.setattr(get_settings(), "openexchangerates_app_id", "environment-app-id")
    await update_provider(
        session, "openexchangerates", {"openexchangerates_app_id": "saved-app-id"}
    )
    app_ids = []
    ...
    client_type = httpx.AsyncClient
    with (
        patch("app.services.fx_rate_service.OpenExchangeRatesProvider", OpenExchangeRatesProvider),
        patch(
            "app.providers.openexchangerates.httpx.AsyncClient",
            side_effect=lambda **kwargs: client_type(
                transport=httpx.MockTransport(respond), **kwargs
        ...
        assert await sync_rates(session) == 0
