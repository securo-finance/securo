...
@pytest.mark.asyncio
async def test_create_settlement_links_existing_transaction(
...
@pytest.mark.asyncio
async def test_create_settlement_invalid_transaction(
    session: AsyncSession, test_user, test_workspace
):
    group, a, b, _c = await _setup_group(session, test_user.id, test_workspace.id)
...
@pytest.mark.asyncio
async def test_create_settlement_account_and_transaction_conflict(
    session: AsyncSession, test_user, test_workspace
):
    group, a, b, _c = await _setup_group(session, test_user.id, test_workspace.id)
    account = await _make_account(session, test_user.id, test_workspace.id)
...
@pytest.mark.asyncio
async def test_create_settlement_with_account_creates_payment_tx(
    session: AsyncSession, test_user, test_workspace
):
    group, a, b, _c = await _setup_group(session, test_user.id, test_workspace.id)
    account = await _make_account(session, test_user.id, test_workspace.id)
...
@pytest.mark.asyncio
async def test_create_settlement_receiver_self_member_owner_fallback(
    session: AsyncSession, test_user, test_workspace
):
...
@pytest.mark.asyncio
async def test_create_settlement_group_not_visible(
    session: AsyncSession, test_user, test_workspace
):
...
@pytest.mark.asyncio
async def test_update_settlement_change_only_from_member(
    session: AsyncSession, test_user, test_workspace
):
...
@pytest.mark.asyncio
async def test_update_settlement_group_not_visible(
    session: AsyncSession, test_user, test_workspace
):
    result = await settlement_service.update_settlement(
        session,
...
@pytest.mark.asyncio
async def test_update_settlement_revalidates_members_and_transaction(
    session: AsyncSession, test_user, test_workspace
):
    group, a, b, c = await _setup_group(session, test_user.id, test_workspace.id)
    account = await _make_account(session, test_user.id, test_workspace.id)
...
@pytest.mark.asyncio
async def test_delete_settlement_group_not_visible(
    session: AsyncSession, test_user, test_workspace
):
    assert (
        await settlement_service.delete_settlement(
...
@pytest.mark.asyncio
async def test_settlement_permission_for_linked_member(
    session: AsyncSession, test_user, test_workspace
):
...
@pytest.mark.asyncio
async def test_update_settlement_permission_denied(
    session: AsyncSession, test_user, test_workspace
):
